      ...
      <head suppressHydrationWarning>
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: `
              // Intercept console.error before Next.js dev overlay loads
              const originalError = console.error;
              console.error = function(...args) {
                const message = args.map(a => typeof a === 'string' ? a : '').join(' ');
                if (message.includes('bis_skin_checked') || message.includes('bis_use') || message.includes('hydrat') || message.includes('Hydrat')) {
                  return;
                }
                originalError.apply(console, args);
              };
              
              // Prevent React's error overlay from catching it
              window.addEventListener('error', function(event) {
                if (event.message && (event.message.includes('bis_skin_checked') || event.message.includes('bis_use') || event.message.includes('hydrat') || event.message.includes('Hydrat'))) {
